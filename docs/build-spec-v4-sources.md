...
| 13 | host `www.wsj.com` section root (`/`, `/finance`, `/business`, `/tech`, `/economy`, `/world`, `/us-news`, `/politics`, `/opinion`, `/markets`, `/personal-finance`, `/real-estate`, `/lifestyle`, `/style`, `/health`, `/sports`, `/arts-culture`) | kind feed via `WSJ_SECTION_FEEDS` on the live host (markets, finance→RSSMarketsMain; business→WSJcomUSBusiness; economy→socialeconomyfeed; tech→RSSWSJD; world, `/`→RSSWorldNews; us-news→RSSUSnews; politics→socialpoliticsfeed; opinion→RSSOpinion; personal-finance→RSSPersonalFinance; real-estate→latestnewsrealestate; lifestyle→RSSLifestyle; style→RSSStyle; health→socialhealth; sports→rsssportsfeed; arts-culture→RSSArtsCulture; every slug verified live 2026-08-20), site wsj. **Listing half** (added 2026-08-20): `/news/<column>` (not rss-news-and-feeds, archive, types, author), `/news/types/<slug>`, `/news/author/<slug>` have no public feed — Dow Jones's own directory carries none for Heard on the Street — so the page itself is the feed the poller reads: kind feed, connector rss, label "WSJ section", feed_url = the page URL, config `{wsj_section}`. The page is Next.js; its pieces (articleUrl, headline, one-line summary, timestamp) sit in the `__NEXT_DATA__` JSON under `pageProps.latestArticles` / `moreInArticlesInitial` / `authorFeedArticles` (`rss.wsj_section_items`, all three shapes verified against live captures 2026-08-19). The listing itself is DataDome-walled, so `rss._listing_text` fetches it with wall detection ON through `fetch.get`, handing the browser path `WSJ_LISTING_MARKERS` in the article body markers' place (the sidecar's wall wait ends when the listing JSON has rendered, and `detect_wall` reads the markers against the full text since the JSON sits at the tail). Item pages then ride the ordinary WSJ path: full text through the sidecar, the listing's one-line summary as the teaser when an article walls, "Sign-in needed" when the listing itself walls, and "No articles found on the page" when a page answers without the listing shape (a redesign never reads as a healthy source). |
