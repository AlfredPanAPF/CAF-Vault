...
- `graph/router.py`: 25 ordered rules turn one pasted URL into a source kind (YouTube, X, FT, WSJ, Substack incl. custom domains, podcasts, rss-bridge sites, raw feeds, article pages, ...). Our router owns detection; rss-bridge `findfeed` is asked last.
- `graph/watchlist.py`: ticker-only watchlist, metadata from yfinance with SEC fallback; EDGAR polls SEC registrants only.
- `graph/credentials.py`, `fetch.py`, `probes.py`: per-site cookies.txt / bearer stored in the `credential` table; `curl_cffi` Chrome impersonation; per-site wall detection raises `SignInNeeded`. Credential values never leave the API, are never logged, and never appear in artifacts or error messages.
